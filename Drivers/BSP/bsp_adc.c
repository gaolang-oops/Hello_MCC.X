...
static ADC_TimeBase_Callback_t s_cb50us = NULL;
static ADC_TimeBase_Callback_t s_cb1ms  = NULL;
 ...
 *   2) 回调注册（在 ISR 内直接调用，须短小）
 */
void BSP_ADC_TimeBase_Tick(void) {
	/* ---- 50us 时基（每个 ISR） ---- */
	s_tick50us++;
	s_flag50us = true;
	if (s_cb50us) {
...
static int16_t RawToMa(uint16_t raw)
{
...
int16_t BSP_ADC_GetCurrentIaMa(void)   { return RawToMa(s_adcData.raw_ia); }
int16_t BSP_ADC_GetCurrentIbMa(void)   { return RawToMa(s_adcData.raw_ib); }
int16_t BSP_ADC_GetCurrentIcMa(void)   { return RawToMa(s_adcData.raw_ic); }

/* Ibus 单极性换算：GND 参考无偏置，0A → raw=0，故不减中点、不取符号翻转。
 * 例：raw=0   → mA=0    （静止）
 *     raw=310 → mA=999  （约 1A）
 *     raw=931 → mA=3000 （满量 3A） */
static int16_t RawToMaIbus(uint16_t raw)
{
    return (int16_t)(__builtin_muluu(raw, ADC_CURRENT_K_IBUS) >> ADC_CURRENT_SHIFT);
}
int16_t BSP_ADC_GetCurrentIbusMa(void) { return RawToMaIbus(s_adcData.raw_ibus); }
