...
/**
   @Summary
     按 angle16（16位二进制弧度）查表 + 两点线性插值，返回 Q1.15 正弦 + 余弦。

   @Description
     SPWM 相位累加器的标准查表入口。angle16 高 7 位（>>9）即 128 点表索引，
     uint16 天然回卷，负角度补码自动正确（无需分支）。
     表上点（frac=0）返回表值本身；非表上点在相邻两点间做线性插值，
     乘积直接右移取整（截断，无舍入偏置），全角度对真实 sin/cos 最大
     误差 ~11.1 LSB（截断查表为 1608）。
      cos θ = sin(θ + 90°)：角度域 +0x4000（uint16 加法溢出免费回卷）。
     90° 为 2 的幂分点 → 索引平移无量化误差；且角度平移 +0x4000 不改变
     低 9 位 → cos 与 sin 小数相位相同，两通道共享同一份 frac/权重。
...
/* ---- 汇编版 sin/cos 一体查表+插值（sincos_asm.s）----
 * 与 C 版 SinCos16 算法一致、结果逐比特相同，供热路径选型/速度对比。
 * 算法: sin_idx = (angle16 × 128) >> 16,  frac16 = 乘积低 16 位
 ...
 *        360°，回卷免费且免第二次乘法）
 * 两路径数值恒等（frac=0 时插值项本就为 0），C 版无需对应分支。
 */
